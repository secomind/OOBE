  ...
  useEffect(() => {
    ...
    const processImage = async () => {
      try {
        ...
        const data = await apiClient.getBlisterPackResult(file, analysisMode);
      ...
      <div className="row flex-grow-1 align-items-center mb-5">
        ...
        <div className="col-md-5 d-flex flex-column align-items-center justify-content-center">
          ...
          <div className="d-flex flex-wrap justify-content-center gap-3 mt-auto mb-4">
            <Button
              variant="light"
              className={`analysis-result-button py-2 px-3 fw-bold`}
            ...
            <Button
              ...
              className={`analysis-result-button py-2 px-4 fw-bold ${status === "analysis" && analysisMode === "cpu" ? "active-analysis" : ""}`}
              onClick={() => {
                setStatus("analysis");
                setAnalysisMode("cpu");
              }}
            ...
            <Button
              variant="light"
              disabled={status === "analysis"}
              className={`analyze-gpu-button py-2 px-4 fw-bold ${status === "analysis" && analysisMode === "gpu" ? "active-analysis" : ""}`}
              onClick={() => {
                setStatus("analysis");
                setAnalysisMode("gpu");
              }}
            ...
            <Button
              variant="light"
              disabled={status === "analysis"}
              className={`analyze-npu-button py-2 px-4 fw-bold ${status === "analysis" && analysisMode === "npu" ? "active-analysis" : ""}`}
              onClick={() => {
                setStatus("analysis");
                setAnalysisMode("npu");
              }}
